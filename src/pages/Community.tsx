    ...
    setAlbums(albums.map(album => {
      if (album.id === id) {
        if (voteType === 'up') {
          ...
          return {
            ...album,
            upvotes: album.upvotes + 1
          };
        } else {
          toast.info("Downvoted soccer album");
          return {
            ...album,
            downvotes: album.downvotes + 1
          };
        }
      }
      return album;
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center">
            ...
            <h1 className="text-3xl font-bold">Albums Ranking</h1>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentAlbums.map(album => <Card key={album.id} className="bg-[#1F1F1F] border-[#333] text-white overflow-hidden hover:border-green-500 transition-all">
              <div className="relative bg-[#2A2A2A]">
                <AspectRatio ratio={1 / 1}>
                  <img src={album.imageUrl} alt={album.title} className="w-full h-full object-contain" onLoad={() => handleImageLoad(album.imageUrl)} onError={() => handleImageError(album.imageUrl)} style={{
              ...
              <CardContent className="pt-4">
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {album.tags.map(tag => <Badge key={tag} variant="outline" className="text-xs bg-green-900/20 text-green-400 border-green-900">
                      {tag}
                    </Badge>)}
                </div>
              </CardContent>
              ...
              <CardFooter className="pt-0 flex justify-between">
                <div className="flex gap-4">
                  <button onClick={() => handleVote(album.id, 'up')} className="flex items-center gap-1 text-gray-400 hover:text-green-500">
                    <ThumbsUp size={18} />
                    <span>{album.upvotes}</span>
                  </button>
                  
                  <button onClick={() => handleVote(album.id, 'down')} className="flex items-center gap-1 text-gray-400 hover:text-red-500">
                    <ThumbsDown size={18} />
        ...
        {totalPages > 1 && <Pagination className="mt-8">
            <PaginationContent className="text-white">
              {currentPage > 1 && <PaginationItem>
                  <PaginationPrevious onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} className="bg-[#1F1F1F] text-white border-[#FFEB3B] hover:bg-[#FFEB3B] hover:text-black transition-colors" />
                </PaginationItem>}
              
              {Array.from({
            length: totalPages
          }, (_, i) => i + 1).map(page => <PaginationItem key={page}>
                  <PaginationLink isActive={currentPage === page} onClick={() => setCurrentPage(page)} className={currentPage === page ? "bg-[#FFEB3B] text-black border-[#FFEB3B] hover:bg-[#FFEB3B]/90" : "bg-[#1F1F1F] text-white border-[#FFEB3B] hover:bg-[#FFEB3B] hover:text-black transition-colors"}>
                    {page}
                  </PaginationLink>
                </PaginationItem>)}
              
              {currentPage < totalPages && <PaginationItem>
                  <PaginationNext onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} className="bg-[#1F1F1F] text-white border-[#FFEB3B] hover:bg-[#FFEB3B] hover:text-black transition-colors" />
                </PaginationItem>}
            </PaginationContent>
          </Pagination>}
      </main>
