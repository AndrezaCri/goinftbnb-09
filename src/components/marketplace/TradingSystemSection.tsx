...
const availableTrades = [{
  ...
  offering: {
    name: "Rising Star",
    rarity: "Common",
    image: "/lovable-uploads/80efbc91-4123-4701-85c5-3b444308e344.png"
  },
  requesting: {
    name: "Midfield Maestro",
    rarity: "Rare",
      ...
      <Tabs defaultValue="my-cards" className="w-full">
          ...
          <div className="space-y-4">
            <div className="flex gap-4 items-center">
              <div className="flex-1">
                <Input placeholder="Buscar seus cards..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-[#111] border-[#333] text-white placeholder:text-white" />
              </div>
              <Button variant="outline" className="border-[#333] text-white bg-zinc-900 hover:bg-zinc-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCards.map(card => <Card key={card.id} className="bg-[#111] border-[#333] overflow-hidden">
                  <div className="p-4">
                    <AspectRatio ratio={1 / 1}>
                      <img src={card.image} alt={card.name} className="rounded-md object-cover w-full h-full" />
                    </AspectRatio>
                  </div>
                  <CardContent>
                    ...
                    {card.isTrading && <Badge variant="outline" className="border-green-500 text-green-500">
                        Em negociação
                      </Badge>}
                  </CardContent>
                  <CardFooter className="bg-[#0a0a0a] border-t border-[#333] p-4">
                    <Button className="w-full" disabled={card.isTrading} variant={card.isTrading ? "outline" : "default"} onClick={() => !card.isTrading && handleOfferTrade(card.id)}>
        ...
        <TabsContent value="available-trades">
          <div className="space-y-6">
            <div className="flex gap-4 items-center">
              <div className="flex-1">
                <Input placeholder="Buscar trocas disponíveis..." className="bg-[#111] border-[#333] text-white placeholder:text-white" />
              </div>
            ...
            <div className="space-y-4">
              {availableTrades.map(trade => <Card key={trade.id} className="bg-[#111] border-[#333]">
                  <CardHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      ...
                      <div className="text-center">
                        ...
                        <div className="bg-[#1a1a1a] rounded-lg p-3">
                          <img src={trade.offering.image} alt={trade.offering.name} className="w-20 h-20 mx-auto rounded-md mb-2" />
                      ...
                      <div className="text-center">
                        ...
                        <div className="bg-[#1a1a1a] rounded-lg p-3">
                          <img src={trade.requesting.image} alt={trade.requesting.name} className="w-20 h-20 mx-auto rounded-md mb-2" />
                  ...
                  <CardFooter className="bg-[#0a0a0a] border-t border-[#333] p-4">
                    <div className="flex gap-2 w-full">
                      <Button variant="outline" className="flex-1 border-[#333] text-white" onClick={() => handleViewProfile(trade.user)}>
                        Ver Perfil
                      </Button>
                      <Button className="flex-1" onClick={() => handleProposeTrade(trade.id)}>
                        Propor Troca
                      </Button>
                    </div>
                  </CardFooter>
